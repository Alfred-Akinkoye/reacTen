...
def WaitPage(root,button1,button2):
    global mode
    if(button2):
...
#makes a container that holds the button(used for initial screen size)
  ...
  button3 = tk.Button(messframe,text = " Continue",font = 40,  fg =  'black',state = DISABLED, command = lambda: WaitPage(root,button1.invoke(),button2.invoke()))
